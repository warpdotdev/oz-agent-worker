...
- `volume_mount_timeout` controls how long a Pod may keep failing to mount a volume (`FailedMount`) before the task is failed; the kubelet retries mounts on its own, so transient cluster conditions do not fail the task immediately. It defaults to `2m`, and `0s` disables failing the task on mount failures altogether
