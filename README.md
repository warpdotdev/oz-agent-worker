...
- `volume_mount_timeout` controls how long a Pod may keep failing to mount a volume (`FailedMount`) before the task is failed. The kubelet retries mounts on its own, so transient cluster conditions do not fail the task immediately: the window is measured from the first mount-failure event, and a pod that has stopped reporting mount failures is treated as recovered. It defaults to `2m`, and `0s` disables failing the task on mount failures altogether
