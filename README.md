...
- set `sidecar_copy_readiness: true` (Helm: `kubernetesBackend.sidecarCopyReadiness`) to start the next init or task when copying succeeds, while retaining disk-backed payload volumes. This opt-in mode uses [native sidecars](https://kubernetes.io/docs/concepts/workloads/pods/sidecar-containers/) with startup probes; Kubernetes 1.33+ has stable support, and 1.29–1.32 requires `SidecarContainers` enabled. It cannot be combined with `use_image_volumes`. The startup preflight checks that a Job can complete while its copy helper stays alive
- in readiness mode, both tar processes must succeed before startup proceeds. An interrupted partial copy is cleared before another attempt. A completed helper restarts without copying again, preserving writable payloads. Copy errors persist in private per-copy state and fail the task; their Jobs are deleted after logs are collected unless cleanup is disabled (`--no-cleanup` or `cleanup: false`). Normal helper termination after task completion does not fail the task. Job deletion stops the helpers too; worker shutdown/context cancellation retains the existing Job-preservation behavior
- readiness helpers retain their container resource allocations during task execution. Namespace defaults, if any, therefore apply alongside the task's requests rather than only during initialization. Startup probes check once per second and restart a helper after ten minutes without successful copying; use `active_deadline_seconds` to bound total task lifetime. `sidecar_prep` timing uses the next init's start as an upper bound (including that image pull), or the Pod's `Initialized` transition when copies are the last init containers
