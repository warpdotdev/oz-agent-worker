...
func kubernetesTaskWrapperScript() string {
	return strings.Join([]string{
		...
		"    return $?",
		"  fi",
		"  /agent/entrypoint.sh harness-support --run-id \"$OZ_RUN_ID\" report-shutdown --error-category agent_process_failed --error-message \"agent process exited with status $status\"",
		"}",
		"",
		"shutdown_signal=\"\"",
		"forward_signal() {",
		"  shutdown_signal=\"$1\"",
		"  if [ -n \"${agent_pid:-}\" ]; then",
		"    kill -\"$shutdown_signal\" \"$agent_pid\" 2>/dev/null || true",
		"  fi",
		"}",
		"",
		...
		"  wait \"$agent_pid\"",
		"  signal_status=\"$?\"",
		"  if [ \"$signal_status\" -ne 127 ]; then",
		"    status=\"$signal_status\"",
		"  fi",
		"fi",
		"trap - TERM INT",
		"",
		"if ! report_shutdown \"$status\"; then",
		"  echo \"warning: failed to report agent shutdown\" >&2",
		"fi",
		"exit \"$status\"",
	}, "\n")
}
