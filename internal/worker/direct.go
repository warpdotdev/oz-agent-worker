...
type DirectBackend struct {
	...
	// capturesMutex guards captures, the exact (task, execution) to capture
	// registry a debug-archive request resolves against. Direct execution has
	// no provider log API, so its bounded capture is the only log source.
	capturesMutex sync.Mutex
	captures      map[executionKey]*debuglog.TaskLogCapture
}
...
func NewDirectBackend(ctx context.Context, config DirectBackendConfig) (*DirectBackend, error) {
	...
	return &DirectBackend{
		config:   config,
		ozPath:   ozPath,
		captures: make(map[executionKey]*debuglog.TaskLogCapture),
	}, nil
}
...
func (b *DirectBackend) ExecuteTask(ctx context.Context, params *TaskParams) ExecuteResult {
	...
	// Registering the capture before any setup work runs means output from the
	// very first phase is archivable, and a request arriving mid-execution
	// resolves this exact execution's capture.
	executionID := taskExecutionID(params)
	phases := newDirectPhaseWriters(params.LogCapture)
	if params.LogCapture != nil {
		b.registerCapture(taskID, executionID, params.LogCapture)
	}
	...
	defer func() {
		if usingTargetDir {
			...
			b.runTeardownIfConfigured(ctx, taskID, workspaceDir, gitConfigPath, phases)
			return
		}
		...
		b.cleanup(ctx, taskID, workspaceDir, gitConfigPath, phases)
	}()
	...
	if b.config.SetupCommand != "" {
		...
		if err := b.runCommand(ctx, b.config.SetupCommand, workspaceDir, setupEnv, phases.setup); err != nil {
			if ctx.Err() != nil {
	...
	cmd.Stdout = phases.agent.stdout
	cmd.Stderr = phases.agent.stderr
...
func (b *DirectBackend) runTeardownIfConfigured(ctx context.Context, taskID, workspaceDir, gitConfigPath string, phases directPhaseWriters) {
	...
	if err := b.runCommand(ctx, b.config.TeardownCommand, workspaceDir, teardownEnv, phases.teardown); err != nil {
...
func (b *DirectBackend) cleanup(ctx context.Context, taskID, workspaceDir, gitConfigPath string, phases directPhaseWriters) {
	b.runTeardownIfConfigured(ctx, taskID, workspaceDir, gitConfigPath, phases)
...
func (b *DirectBackend) runCommand(ctx context.Context, command, dir string, env []string, output phaseWriters) error {
...
// phaseWriters are the stdout and stderr sinks for one execution phase.
type phaseWriters struct {
	stdout io.Writer
	stderr io.Writer
}

// directPhaseWriters routes each phase's output to the worker console and,
// when archive capture is enabled, to the task's bounded capture as well.
type directPhaseWriters struct {
	setup    phaseWriters
	agent    phaseWriters
	teardown phaseWriters
}

func newDirectPhaseWriters(capture *debuglog.TaskLogCapture) directPhaseWriters {
	console := phaseWriters{stdout: os.Stdout, stderr: os.Stderr}
	if capture == nil {
		return directPhaseWriters{setup: console, agent: console, teardown: console}
	}

	phase := func(p debuglog.Phase) phaseWriters {
		return phaseWriters{
			stdout: io.MultiWriter(os.Stdout, capture.Writer(p, debuglog.StreamStdout)),
			stderr: io.MultiWriter(os.Stderr, capture.Writer(p, debuglog.StreamStderr)),
		}
	}
	return directPhaseWriters{
		setup:    phase(debuglog.PhaseSetup),
		agent:    phase(debuglog.PhaseAgent),
		teardown: phase(debuglog.PhaseTeardown),
	}
}

func (b *DirectBackend) registerCapture(taskID, executionID string, capture *debuglog.TaskLogCapture) {
	b.capturesMutex.Lock()
	defer b.capturesMutex.Unlock()
	b.captures[executionKey{runID: taskID, executionID: executionID}] = capture
}

func (b *DirectBackend) lookupCapture(taskID, executionID string) (*debuglog.TaskLogCapture, bool) {
	b.capturesMutex.Lock()
	defer b.capturesMutex.Unlock()
	capture, ok := b.captures[executionKey{runID: taskID, executionID: executionID}]
	return capture, ok
}

// SnapshotTaskLogs replays this execution's capture into sink at a fixed
// watermark, so an active task keeps writing while the snapshot is taken and
// its later output remains available to a later request.
func (b *DirectBackend) SnapshotTaskLogs(_ context.Context, params *SnapshotParams) error {
	capture, ok := b.lookupCapture(params.TaskID, params.ExecutionID)
	if !ok || capture == nil {
		return debuglog.NewSnapshotError(types.DebugArchiveReasonCaptureUnavailable, "no output capture exists for this execution")
	}
	if err := capture.SnapshotTo(params.Sink); err != nil {
		return debuglog.NewSnapshotError(types.DebugArchiveReasonSnapshotFailed, "failed to replay the output capture")
	}
	return nil
}

// CleanupTaskResources forgets the execution's capture. The worker owns the
// capture handle and closes it, which is what deletes its bytes from disk.
func (b *DirectBackend) CleanupTaskResources(_ context.Context, params *CancelParams) error {
	b.capturesMutex.Lock()
	defer b.capturesMutex.Unlock()
	delete(b.captures, executionKey{runID: params.TaskID, executionID: params.ExecutionID})
	return nil
}
